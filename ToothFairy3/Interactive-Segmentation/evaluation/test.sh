...
LABELS_DIR="$TEST_DATA_DIR/labelsTr_interactive"
...
echo "Running ToothFairy3 Interactive-Segmentation evaluation..."
...
if [ $? -eq 0 ]; then
    ...
    if [ -f "$METRICS_FILE" ]; then
        ...
        if command -v jq >/dev/null 2>&1; then
            if jq -e '.aggregates.Dice_AUC.mean' "$METRICS_FILE" >/dev/null 2>&1; then
                echo "Average Dice AUC: $(jq -r '.aggregates.Dice_AUC.mean' "$METRICS_FILE")"
            fi
            if jq -e '.aggregates.HD95_AUC.mean' "$METRICS_FILE" >/dev/null 2>&1; then
                echo "Average HD95 AUC: $(jq -r '.aggregates.HD95_AUC.mean' "$METRICS_FILE")"
            fi
            if jq -e '.aggregates.Dice_Final.mean' "$METRICS_FILE" >/dev/null 2>&1; then
                echo "Average Dice after last click: $(jq -r '.aggregates.Dice_Final.mean' "$METRICS_FILE")"
            fi
            if jq -e '.aggregates.HD95_Final.mean' "$METRICS_FILE" >/dev/null 2>&1; then
                echo "Average HD95 after last click: $(jq -r '.aggregates.HD95_Final.mean' "$METRICS_FILE")"
            fi
        else
