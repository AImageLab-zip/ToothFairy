...
def select_reliable_series_nnunet(out_csv_path,out_txt_path, score_thres=0.9, num_selected=1000):
    ...
    bottleneck_thresh = 100
    for score in all_df:
        if float(score[1]) > score_thres and float(score[2]) == 2:
...
def selected_img_save(img_path,iteration_save_path,txt_file):
    ...
    print(pseudo_cases)
    bad_case_path = r'/media/ps/lys/CBCT_IAN/Fairy_Tooth/experiment_newdata_val/iter0_data_filter/image_bad'

    for c in [i for i in pseudo_cases if i.replace('.nii.gz','_0000.nii.gz') not in os.listdir(bad_case_path)]:
...
if __name__ == "__main__":
    ...
    model_list = ['333','666','1000','best']

    model_dir_1 = r'/media/ps/lys/CBCT_IAN/Fairy_Tooth/experiment_newdata_val/iter2_data_filter/pseudo_infer'
    ...
    # model_list = ['333', '666','1000' ,'best']
    #
    # model_dir_2 = r'/media/ps/lys/CBCT_IAN/Fairy_Tooth/experiment_newdata_val/iter1_data_filter/pseudo_infer_SDA/nnunetv2'
    #
    # files_name = [i for i in os.listdir(os.path.join(model_dir_2, model_list[-1])) if i.endswith('.gz')]

    # models_reliable_compute(model_dir_2, files_name, os.path.join(model_dir_2, 'reliable_score.csv'), model_list, num_class=1)

    result_path = r'/media/ps/lys/CBCT_IAN/Fairy_Tooth/experiment_newdata_val/iter2_data_filter/pseudo_infer/result'

    select_reliable_series_nnunet(os.path.join(model_dir_1,'reliable_score.csv'),result_path,score_thres=0.9, num_selected=50)

    img_path = r'/media/ps/lys/CBCT_IAN/Fairy_Tooth/experiment_newdata_val/iter2_data_filter/image_unlabeled'
    iteration_save_path = r'/media/ps/lys/CBCT_IAN/Fairy_Tooth/experiment_newdata_val/iter2_data_filter'

    selected_img_save(img_path,iteration_save_path,os.path.join(result_path,'reliable_series_100.txt'))

    image_path = os.path.join(iteration_save_path, 'image_selected')
    label_path = os.path.join(model_dir_1,'best')
    process_path = os.path.join(iteration_save_path, 'SDA')
    # strong_data_augment(image_path,label_path,process_path)
