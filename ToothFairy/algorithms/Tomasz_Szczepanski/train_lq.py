...
from src.arg_parser_lq import args
from natsort import natsorted

tuple2str = lambda a: '_'.join([str(x) for x in list(a)])
model_folder_name = "RES"
args.cache_dir = os.path.join(args.cache_dir, f"LQ_SYNTH2_{model_folder_name}_{args.autocast_dtype}_{tuple2str(args.patch_size)}")
...
# from src.loss import AngularLoss
...
data_dicts_list = [{args.keys[0]: image, args.keys[1]: dense_synthethic} for (image, dense_synthethic) in zip(image_paths, synthetic_paths)]
    ...
    with experiment.train():
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            with torch.no_grad():
                ...
                directory = os.path.join(args.checkpoint_dir, model_folder_name, f"LQ_SYNTH2_folds_{args.split}_{args.model_name}_{args.autocast_dtype}_nearest")
