...
from src.arg_parser_gcr_pretrain import args
from natsort import natsorted

tuple2str = lambda a: '_'.join([str(x) for x in list(a)]) 
model_folder_name = "ResUNet"
data_type = "SYNTH2" if args.use_synthetic else "DENSE_GT"
args.cache_dir = os.path.join(args.cache_dir, f"HQ_{data_type}_{model_folder_name}_2CH_{args.autocast_dtype}_{tuple2str(args.patch_size)}")
...
if args.comet:
    ...
    experiment.log_asset('src/arg_parse_gcr_pretrain.py')
...
if args.use_json_splits:
    ...
    if "ToothFairy" in args.data:
        if not args.use_synthetic:
            ...
            if args.debug:
                data_ids = list(range(int(len(synthetic_paths)*0.15)))
            else:
                data_ids = list(range(int(len(synthetic_paths)*1.0)))
            train_split_ids, test_ids = train_test_split(data_ids, test_size=0.1, shuffle=True, random_state=48)
            val_split_ids, test_split_ids = train_test_split(test_ids, test_size=0.5, shuffle=True, random_state=48)
else:
    with open(os.path.join(args.data.replace('/Dataset', '/'), 'splits.json')) as splits_file:
        json_splits = json.load(splits_file)
    synthethic_patients = json_splits["synthetic"]
...
if len(args.keys) == 3:
    if args.use_synthetic:
        ...
        data_dicts_list = [{args.keys[0]: image, args.keys[1]: gcr, args.keys[2]: dense} for (image, gcr, dense) in zip(image_paths, dense_paths, dense_paths)]
else:
...
def training_step(batch_idx, train_data, args):
    ...
    with torch.cuda.amp.autocast(enabled=args.use_scaler, dtype=autocast_d_type):
        if epoch <= 300:
            #replace gcr channels with zeros to force network learn features 
            for i in train_data["image"]:
                #iterate over batch
                # channel 1 == gcr
                i[1] = torch.zeros_like(i[1])
        elif epoch < 400:
            for i in train_data["image"]:
                #iterate over batch
                # channel 1 == gcr
                # zero gcr on random - with probability
...
for fold, (train_ids, val_ids) in enumerate(kfold.split(train_dataset)):
    ...
    if args.continue_training:
        ...
        if model_dict.get('optimizer_state_dict'):
            optimizer.load_state_dict(model_dict['optimizer_state_dict'])
        if model_dict.get('lr_scheduler'):
            scheduler.load_state_dict(model_dict['lr_scheduler'])
        args.start_epoch = model_dict['epoch']
        print(f'Loaded model, optimizer, shceduler, starting with epoch: {args.start_epoch}')
    ...
    with experiment.train():
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            with torch.no_grad():
                ...
                if not args.scheduler_name == "plateau":
                    scheduler.step()
                    experiment.log_metric("lr_rate", scheduler.get_last_lr(), epoch=epoch)
                else:
                    # DONT CHANGE LR FOR PRETRAINING - NO NEED TO EXPLORE LOCAL MINIMUM
                ...
                directory = os.path.join(args.checkpoint_dir, model_folder_name, f"HQ_{data_type}2_{args.model_name}_FP32_95")
