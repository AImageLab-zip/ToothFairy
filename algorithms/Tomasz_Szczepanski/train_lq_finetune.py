...
from src.arg_parser_lq_finetune import args
from natsort import natsorted

random.seed(42)
tuple2str = lambda a: '_'.join([str(x) for x in list(a)])
model_folder_name = "RES"
args.cache_dir = os.path.join(args.cache_dir, f"LQ_SYNTH2_finetune2_{model_folder_name}_{args.autocast_dtype}_{tuple2str(args.patch_size)}")
...
if args.comet:
    ...
    experiment.log_asset('src/arg_parser_lq.py')
...
if args.use_json_splits:
    if "Maxillo" in args.data:
        ...
        print(f"Found: {len(image_paths)} scans, {len(dense_paths)} dense labels.")
    ...
    print(f"Found: {len(image_paths)} scans,  {len(synthetic_paths)} synthethic dense labels.")

data_dicts_list = [{args.keys[0]: image, args.keys[1]: dense} for (image, dense) in zip(image_paths, dense_paths)]

train_dataset = PersistentDataset(data_dicts_list, trans.train_transform, cache_dir=os.path.join(args.cache_dir, 'train'))
val_dataset = PersistentDataset(data_dicts_list, trans.val_transform, cache_dir=os.path.join(args.cache_dir, 'val'))
test_dataset = PersistentDataset(data_dicts_list, trans.val_transform, cache_dir=os.path.join(args.cache_dir, 'test'))
...
for fold, (train_ids, val_ids) in enumerate(kfold.split(train_dataset)):
    ...
    if args.use_json_splits:
        train_ids = test_split_ids + train_split_ids
        random.shuffle(train_ids)
        val_ids = val_split_ids
        test_ids = None
        # test_ids = test_split_ids
        print(f"Using data split based on splits.json: {len(train_ids)} train, {len(val_ids)} val")
        if fold < 1:
            continue
    else:
        test_ids = None
    ...
    train_loader = ThreadDataLoader(train_dataset, use_thread_workers=True, buffer_size=1, batch_size=args.batch_size, sampler=train_subsampler)#, worker_init_fn=np.random.seed(42))
   ...
   #MODEL INIT
    ...
    if args.finetune_training:
        def set_bn_eval(module):
            if isinstance(module, torch.nn.modules.batchnorm._BatchNorm):
                module.eval()
        mode_path = f"/home/tf/Checkpoints/ToothFairy/ResUNet/LQ_SYNTH2_folds_5_ResUnet18_tfloat32_nearest/model-ResUnet18-fold-{fold}_current_best_val.pt"
        model_dict = torch.load(mode_path, map_location=device)
        ...
        print(f'Loaded model for fine tuning - fold:{fold}.')
    ...
    with experiment.train():
        ...
        for epoch in range(args.start_epoch, args.epochs):
            ...
            with torch.no_grad():
                #validation dataset
                if (epoch+1) % args.validation_interval == 0 or epoch == 1:
                ...
                directory = os.path.join(args.checkpoint_dir, model_folder_name, 'LQ_finetune2', f"LQ_FINETUNING_folds_{args.split}_{args.model_name}_{args.autocast_dtype}")
                if not os.path.exists(directory):
                    os.makedirs(directory)
                # save best TRAIN model
                if best_dice_score < train_metrics_agg[0]:
                    save_path = f"{directory}/model-{args.model_name}-fold-{fold}_current_best_train.pt"
                ...
                if epoch % args.save_interval == 0 and epoch != 0:
                    save_path = f"{directory}/model-{args.model_name}-fold-{fold}_train_{train_metrics_agg[0]:.4f}_epoch_{(epoch+1):04}.pt"
                    ...
                    print(f"Interval model saved! - train_daice: {train_metrics_agg[0]:.4f}, val_dice: {val_metrics_agg[0]:.4f}, best_val_dice: {best_dice_val_score:.4f}.")
