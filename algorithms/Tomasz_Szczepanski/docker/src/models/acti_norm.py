...
class ADN(nn.Sequential):
    ...
    def __init__(
        ...
        if norm is not None:
            ...
            if norm == "group":
                if in_channels==1:
                    op_dict["N"] = get_norm_layer(name=(norm, {"num_groups": 1}), spatial_dims=norm_dim or dropout_dim, channels=in_channels)
                else:
                    op_dict["N"] = get_norm_layer(name=(norm, {"num_groups": 8}), spatial_dims=norm_dim or dropout_dim, channels=in_channels)
            else:
