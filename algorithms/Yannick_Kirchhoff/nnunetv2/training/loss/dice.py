...
class SoftSkeletonDiceLoss(nn.Module):
    """
    This is an implementation of the clDice loss as implemented in
    https://github.com/jocpae/clDice adapted to the nnUNet framework
    """
    def __init__(self, apply_nonlin: Callable = None, batch_dice: bool = False, do_bg: bool = True, smooth: float = 1.,
                 ddp: bool = True):
        super(SoftSkeletonDiceLoss, self).__init__()

        if do_bg:
            raise RuntimeError("skeleton dice does not work with background")
        self.batch_dice = batch_dice
        self.apply_nonlin = apply_nonlin
        self.smooth = smooth
        self.ddp = ddp
    ...
    def forward(self, x, y, loss_mask=None):
        ...
        with torch.no_grad():
            ...
            y_onehot = y_onehot[:, 1:]
            y_skel = soft_skel(y_onehot)
            sum_gt = y_skel.sum(axes) if loss_mask is None else (y_skel * loss_mask).sum(axes)

        x_skel = soft_skel(x)

        sum_pred = x_skel.sum(axes) if loss_mask is None else (x_skel * loss_mask).sum(axes)

        inter_rec = (x * y_skel).sum(axes) if loss_mask is None else (x * y_skel * loss_mask).sum(axes)
        inter_prec = (x_skel * y_onehot).sum(axes) if loss_mask is None else (x_skel * y_onehot * loss_mask).sum(axes)

        if self.ddp and self.batch_dice:
            inter_rec = AllGatherGrad.apply(inter_rec).sum(0)
            inter_prec = AllGatherGrad.apply(inter_prec).sum(0)
            sum_pred = AllGatherGrad.apply(sum_pred).sum(0)
            sum_gt = AllGatherGrad.apply(sum_gt).sum(0)

        if self.batch_dice:
            inter_rec = inter_rec.sum(0)
            inter_prec = inter_prec.sum(0)
            sum_pred = sum_pred.sum(0)
            sum_gt = sum_gt.sum(0)

        rec = (inter_rec + self.smooth) / (torch.clip(sum_gt+self.smooth, 1e-8))
        prec = (inter_prec + self.smooth) / (torch.clip(sum_pred+self.smooth, 1e-8))

        dc = prec * rec / (torch.clip(prec + rec, 1e-8))
